  ...
  return (
    <article className="rounded-card border border-hairline bg-surface-2 px-4 py-4 sm:px-5">
      ...
      <p className="mt-1 text-small text-ink-3">
        {haul.km.toLocaleString("en-IN")} km great-circle · {haul.tonnes} t indicative payload
        {seaKm != null ? ` · sea lane ~${seaKm.toLocaleString("en-IN")} km` : ""}
      </p>
      ...
      <p className="mt-3 text-label text-ink-4">{haul.honestyNote}</p>
    </article>
  );
}
