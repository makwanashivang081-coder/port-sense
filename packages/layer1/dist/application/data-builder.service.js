...
export class DataBuilderService {
    ingest = new IngestionEngine();
    mapper = new SchemaMappingEngine();
    entities = new EntityResolutionEngine();
    units = new UnitNormalizationEngine();
    validator = new ValidationEngine();
    provenance = new ProvenanceEngine();
    buildFromFile(filePath, opts = {}) {
        try {
    ...
    buildFromBytes(fileName, bytes, opts = {}) {
        try {
    ...
    buildFromDataset(artifact, dataset, batchId) {
        const { mappings, rows, missingRequired } = this.mapper.mapDataset(dataset);
        const reviewNotes = [];
        const errors = [];
        if (missingRequired.length > 0) {
            errors.push(`Missing required canonical columns: ${missingRequired.join(", ")}`);
            return {
        ...
        for (const row of rows) {
            ...
            const fp = fingerprint(resolution.canonicalPortId, row.values.observation_date ?? "", waiting?.normalized);
            ...
            const prov = this.provenance.build(artifact, status, row.values.observation_date ?? null);
            records.push({
        ...
        if (accepted === 0 && suspicious === 0) {
            ...
            reviewNotes.push("Some rows invalid/suspicious/duplicate — review before L2 accept");
        }
        else {
            decision = "APPROVED";
        }
...
function fingerprint(portId, date, waiting) {
    return createHash("sha256")
        .update(`${portId}|${date}|${String(waiting)}`)
        .digest("hex");
}
//# sourceMappingURL=data-builder.service.js.map