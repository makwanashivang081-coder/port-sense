...
export class DecisionEngine {
    builder;
    comparator;
    constructor(builder, comparator) {
        this.builder = builder;
        this.comparator = comparator;
    }
    decideForDestination(dest, req) {
        const lanes = this.builder.buildForDestination(dest);
        for (const l of lanes)
            this.builder.assertValidLane(l);
        ...
        if (!winner) {
            ...
            const saveTxt = saveInrVsRunnerUp > 0
    ...
    compareByPriority(a, b, priority) {
        switch (priority) {
            case "lowest_cost":
            ...
            case "lowest_risk":
                return ((RISK_RANK[a.riskLevel] ?? 9) - (RISK_RANK[b.riskLevel] ?? 9) ||
                    a.demurrageInr - b.demurrageInr);
            case "balanced":
            default: {
                // cost rank + risk rank
                const score = (s) => s.demurrageInr / 1000 + (RISK_RANK[s.riskLevel] ?? 9) * 50;
                return score(a) - score(b);
            }
        }
    }
}
export function assertNoDuplicateLanes(lanes) {
