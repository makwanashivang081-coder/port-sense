...
export class AdvisorService {
    advise(input) {
        const spreadsheet = input.rows.map((r) => ({
            origin: r.originName,
        ...
        if (pick) {
            ...
            bullets.push({
                ...
                text: input.saveInrVsRunnerUp != null && input.saveInrVsRunnerUp > 0
            ...
            if (winnerRow && closest && closest.originName !== pick) {
                ...
                const waitShare = winnerRow.totalInr > 0
        ...
        const summary = pick
            ? `For ${input.inlandLabel}: use ${pick}. Add waiting fee + trucking. ${worstWait?.highWait ? `${worstWait.originName} has too much port wait.` : ""}`.trim()
