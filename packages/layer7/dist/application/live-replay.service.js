import { LIVE_INTERVAL_MINUTES, } from "../domain/types.js";
import { analog2023Date } from "./time-clock.service.js";
 ...
 */
export function defaultReplayDate(now = new Date()) {
    const start = Date.UTC(now.getUTCFullYear(), 0, 1);
    const day = Math.floor((now.getTime() - start) / 86_400_000);
    const analog = analog2023Date(`2023-${String(Math.floor(day / 31) + 1).padStart(2, "0")}-01`);
...
export class LiveReplayService {
    ...
    feed(options = {}) {
        ...
        const portId = options.portId ?? "INNSA";
        ...
        const daily = loadDaily2023().days.find((d) => d.date === analog) ?? loadDaily2023().days[0];
        ...
        const nextPostAt = new Date(tick.getTime() + LIVE_INTERVAL_MINUTES * 60_000).toISOString();
        return {
            asOfDate,
            ...
            observations,
            clock,
            agentNote: `Replay agent posts a new observation every ${LIVE_INTERVAL_MINUTES} minutes. ` +
                `Dwell hours are sampled from real JNPA ${analog} events. Temperature is historical Open-Meteo for ${asOfDate}. Not AIS.`,
        };
    }
}
//# sourceMappingURL=live-replay.service.js.map